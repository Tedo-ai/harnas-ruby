  ...
  module Providers
    ...
    class AnthropicStreamLive # rubocop:disable Metrics/ClassLength
      ...
      def call(request, &block)
        state = StreamSupport::AnthropicState.new(turn_id: SecureRandom.uuid, emit: block)
        state.start
      ...
      def consume_chunked_sse(socket, state, &)
        ...
        StreamSupport.dispatch_block(sse_buffer, provider: "anthropic") do |payload|
          state.data(payload)
        end
      end
